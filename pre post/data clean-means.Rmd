...
all.pre.data <- read.csv("/Users/mac/Downloads/TTPreDATA.csv")

all.post.data <- read.csv("/Users/mac/Downloads/TTPostDATA.csv")
```
...
```{r}
#relacing 99, 88 with NA i.e. missing data across the 2 data sets
...
# Calculate mean AMES score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.Ames = rowMeans(select(., starts_with("Pre_AMES")), na.rm = TRUE))

# Calculate mean Self Compassion score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.SelfCompassion = rowMeans(select(., starts_with("Pre_SelfCompassion")), na.rm = TRUE))

# Calculate mean Emotional Clarity score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.EmoClarity = rowMeans(select(., starts_with("Pre_EmoClarity")), na.rm = TRUE)) 
                                       
# Calculate mean Self Esteem score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.SelfEsteem = rowMeans(select(., starts_with("Pre_SelfEsteem")), na.rm = TRUE))

# Calculate mean Black Girl ID Explore score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.BlackGirlId = rowMeans(select(., starts_with("Pre_BlackGirl_ID")), na.rm = TRUE)) 

# Calculate mean Image Acceptance score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.ImgAcceptance = rowMeans(select(., starts_with("Pre_ImgAcceptance")), na.rm = TRUE))

# Calculate mean Relatedness score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.Relatedness = rowMeans(select(., starts_with("Pre_Relatedness")), na.rm = TRUE))


# Calculate mean belonging score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.Belonging = rowMeans(select(., starts_with("Pre_Belonging")), na.rm = TRUE)) 

# Calculate mean Black Girl Space score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.BlackGirlSpace = rowMeans(select(., starts_with("Pre_BlackGirlSpace")), na.rm = TRUE)) 


# Calculate mean Critical Agency score for each participant
all.pre.data <- all.pre.data %>%
  mutate(Total.Pre.CriticalAgency = rowMeans(select(., starts_with("Pre_CriticalAgency")), na.rm = TRUE)) 

```

``` {r}
#recoding certain scales
...
# Calculate mean AMES score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.Ames = rowMeans(select(., starts_with("Post_AMES")), na.rm = TRUE))

# Calculate mean Self Compassion score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.SelfCompassion = rowMeans(select(., starts_with("Post_SelfCompassion")), na.rm = TRUE))

# Calculate mean Emotional Clarity score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.EmoClarity = rowMeans(select(., starts_with("Post_EmoClarity")), na.rm = TRUE))

# Calculate mean Self Esteem score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.SelfEsteem = rowMeans(select(., starts_with("Post_SelfEsteem")), na.rm = TRUE))

# Calculate mean Black Girl ID Explore score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.BlackGirlId = rowMeans(select(., starts_with("Post_BlackGirl_ID")), na.rm = TRUE))

# Calculate mean Image Acceptance score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.ImgAcceptance = rowMeans(select(., starts_with("Post_ImgAcceptance")), na.rm = TRUE))

# Calculate mean Relatedness score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.Relatedness = rowMeans(select(., starts_with("Post_Relatedness")), na.rm = TRUE))

# Calculate mean belonging score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.Belonging = rowMeans(select(., starts_with("Post_Belonging")), na.rm = TRUE))

# Calculate mean Black Girl Space score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.BlackGirlSpace = rowMeans(select(., starts_with("Post_BlackGirlSpace")), na.rm = TRUE))

# Calculate mean Critical Agency score for each participant
all.post.data <- all.post.data %>%
  mutate(Total.Post.CriticalAgency = rowMeans(select(., starts_with("Post_CriticalAgency")), na.rm = TRUE))
```
...
write_xlsx(pre.post.data, "pre_post_data.xlsx")

# The file will be saved in your current working directory
getwd()  # Run this to check where the file is saved
```
